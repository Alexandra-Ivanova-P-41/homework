...
function isPasswordCorrect(password) {
  if (password.length === 6 && password[0] === password[0].toUpperCase() && /[0-9]/g.test(password)) {
    return true;
  }
  return false;
}
...
function resetPassword(email, oldPassword, newPassword) {
  for (let i = 0; i < userDatabase.length; i++) {
    if (userDatabase[i].email === email && oldPassword !== newPassword && isPasswordCorrect(newPassword)) {
...
function validator(value) {
  return {
    ...
    isDate() {
      if (this.isValid && this.object.match(/^\d{1,2}\.\d{1,2}\.\d{4}$/) !== null) {
        this.isValid = true;
        return this;
      }
      this.isValid = false;
    ...
    isFullName() {
      if (this.isValid && this.object.match(/^[А-ЯЁ][а-яё]*(\s[А-ЯЁ][а-яё]*){2}$/)) {
        this.isValid = true;
        return this;
      }
      this.isValid = false;
